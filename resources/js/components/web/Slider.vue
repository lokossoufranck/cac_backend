          ...
          <div class="card">
            ...
            <div class="card-body table-responsive p-0">
              <table class="table table-hover" id="datatable">
                ...
                <tbody>
                  <tr v-for="slider in search()" :key="slider.id">
                    <td>{{ slider.id }}</td>
                    <td>{{ slider.code }}</td>
                    <td>{{ slider.title_1 }}</td>
                    <td>{{ slider.format_name }}</td>
                    <td>
                      <a href="#" @click="download_slider(slider.url)"
                        >Télecharger</a
                      >
                    </td>
                    <td>
        ...
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title" v-show="!editmode">Créer une slider</h5>
              <h5 class="modal-title" v-show="editmode">Modifier une slider</h5>
              <button
                type="button"
                class="close"
                data-dismiss="modal"
            ...
            >
              <div class="modal-body">
                ...
                <div class="form-group">
                  ...
                  <input
                    type="file"
                    @change="onData_sliderChange"
                    name="data_slider"
                    class="form-control form-control-sm"
                    :class="{
                      'is-invalid': form.errors.has('data_slider'),
                    }"
                  />
                  ...
                  <!--  <has-error :form="form" field="code"></has-error> -->
                  <span
                    v-if="errors.data_slider"
                    :class="['invalid-feedback d-block']"
                  >
                    {{ errors.data_slider[0] }}</span
                  >
                </div>
...
export default {
  ...
  methods: {
    search() {
      var results = [];
      var searchData = this.sliders.data;
    ...
    getResults(page = 1) {
      ...
      axios
        .get("/api/slider?page=" + page)
        .then(({ data }) => (this.sliders = data.data));

      this.$Progress.finish();
    },
    //For getting Instant Uploaded Photo1
    onData_sliderChange(e) {
      this.form.data_slider = e.target.files[0];
    ...
    editModal(slider) {
      this.editmode = true;
      this.form.reset();
      if (slider.adresse == null || slider.adresse == undefined) {
        this.isWeb = false;
      } else {
        this.isWeb = true;
        console.log("not web");
    ...
    createSlider() {
      ...
      formData.append("data_slider", this.form.data_slider);
      ...
      axios
        .post("/api/slider", formData, config)
        .then((data) => {
    ...
    updateSlider() {
      ...
      axios
        .post("/api/slider/" + this.form.id, formData, config)
        .then((data) => {
          if (data.data.success) {
            $("#addNew").modal("hide");
            Toast.fire({
