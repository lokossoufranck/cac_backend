        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header">
              <h3 class="card-title">Liste des sections verticales</h3>
            ...
            <div class="card-body table-responsive p-0">
              <table class="table table-hover" id="datatable">
                ...
                <tbody>
                  <tr v-for="sectionv in search()" :key="sectionv.id">
                    <td>{{sectionv.id }}</td>
                    <td>{{sectionv.code }}</td>
                    <td>{{sectionv.title_1 }}</td>
                    <td>{{sectionv.format_name }}</td>
                    <td>
                      <a href="#" @click="download_sectionv(sectionv.url)"
                        >Télecharger</a
                      >
                    </td>
                    <td>
                      <a href="#" @click="editModal(sectionv)">
                        <i class="fa fa-edit blue"></i>
                      </a>
                      /
                      <a href="#" @click="deleteSectionV(sectionv.id)">
      ...
      >
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            ...
            <form
              @submit.prevent="editmode ? updateSectionV() : createSectionV()"
            ...
            >
              <div class="modal-body">
                ...
                <div class="form-group">
                  ...
                  <input
                    type="file"
                    @change="onData_sectionvChange"
                    name="data_sectionv"
                    class="form-control form-control-sm"
                    :class="{
                      'is-invalid': form.errors.has('data_sectionv'),
                    }"
                  />

                  <img
                    v-if="getUrl() != '/section_v_s/'"
                    class="profile-user-img img-fluid"
                    :src="
                      getUrl() != ''
                        ? '/section_v_s/' + getUrl()
                        : '/section_v_s/default.png'
                    "
                  ...
                  <!--  <has-error :form="form" field="code"></has-error> -->
                  <span
                    v-if="errors.data_sectionv"
                    :class="['invalid-feedback d-block']"
                  >
                    {{ errors.data_sectionv[0] }}</span
                  >
                </div>
...
export default {
  ...
  methods: {
    search() {
      var results = [];
      var searchData = this.sectionvs.data;
    ...
    getResults(page = 1) {
      ...
      axios
        .get("/api/sectionv?page=" + page)
        .then(({ data }) => (this.sectionvs = data.data));

      this.$Progress.finish();
    },
    //For getting Instant Uploaded Photo1
    onData_sectionvChange(e) {
      this.form.data_sectionv = e.target.files[0];
    },
    download_sectionv(url) {
      axios({
        url: "/api/sectionv/download/" + url,
        method: "GET",
        ...
        link.download = "sectionv_" + url;
        link.click();
      });
    },
    loadSectionVs() {
      axios.get("/api/sectionv").then(({ data }) => {
        this.sectionvs = data.data;
      });
    },
    loadFormats() {
    ...
    editModal(sectionv) {
      this.editmode = true;
      this.form.reset();
      if (sectionv.adresse == null ||sectionv.adresse == undefined) {
        this.isWeb = false;
      } else {
        this.isWeb = true;
        console.log("not web");
    ...
    createSectionV() {
      ...
      formData.append("data_sectionv", this.form.data_sectionv);
      ...
      axios
        .post("/api/sectionv", formData, config)
        .then((data) => {
    ...
    updateSectionV() {
      ...
      axios
        .post("/api/sectionv/" + this.form.id, formData, config)
        .then((data) => {
          if (data.data.success) {
            $("#addNew").modal("hide");
            Toast.fire({
