        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header">
              <h3 class="card-title">Liste des lessons</h3>
              ...
              <table class="table table-hover" id="datatable">
                ...
                <tbody>
                  <tr v-for="modulef in search()" :key="modulef.id">
                    ...
                    <!-- <td><img v-bind:src="'/' + modulefs.photo" width="100" alt="modulefs"></td> -->
                    <td>
                      ...
                      <a
                        ...
                        @click="deleteLesson(modulef.id, modulef.status)"
                      >
        ...
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title" v-show="!editmode">Créer une lesson</h5>
              <h5 class="modal-title" v-show="editmode">Modifier une lesson</h5>
              <button
                type="button"
                class="close"
                data-dismiss="modal"
            ...
            <form @submit.prevent="editmode ? updateLesson() : createLesson()">
              <div class="modal-body">
                <div class="form-group">
                  ...
                  <select class="form-control" v-model="form.section_id">
                    <option
                      ...
                      :key="index"
                      :value="index"
                      :selected="index == form.section_id"
                    >
                      {{ section.name }}
                    </option>
                  </select>
                ...
                <div class="form-group">
                  ...
                  <vue-tags-input
                    v-model="form.modulef"
                    :tags="form.modulefs"
                    placeholder="Ajouter une modulef"
                    :autocomplete-items="filteredModulefItems"
                    @tags-changed="(newTags) => (form.modulefs = newTags)"
                  />
                  <has-error :form="form" field="modulefs"></has-error>
                </div>

                <div class="form-group">
                  <label> Ressources (vimeo) </label>
                  <vue-tags-input
                    placeholder="Ajouter un objectif"
                    v-model="form.ressource"
                    :tags="form.ressources"
                    :autocomplete-items="filteredRessourceItems"
                    @tags-changed="(newTags) => (form.ressources = newTags)"
                  />
                  <has-error :form="form" field="ressources"></has-error>
                </div>
                ...
                <div class="form-group">
                  ...
                  <input
                    ...
                    class="form-control"
                    :class="{
                      'is-invalid': form.errors.has('content'),
                    }"
                  />
                  <has-error :form="form" field="code"></has-error>
                </div>
...
export default {
  ...
  methods: {
    search() {
      var results = [];
      var searchData = this.lessons.data;
      // var searchData = this.lessons;
      var sparam = this.query.toLowerCase();
      if (this.query == "") {
        this.lessons.data = searchData;
        results = searchData;
      } else {
  ...
  computed: {
    filteredModulefItems() {
      return this.autocompleteFormationItems.filter((i) => {
        return i.text.toLowerCase().indexOf(this.modulef.toLowerCase()) !== -1;
      });
    },

    filteredRessourceItems() {
      return this.autocompleteWhatslearnItems.filter((i) => {
        return (
          i.text.toLowerCase().indexOf(this.ressource.toLowerCase()) !== -1
        );
      });
    },
  },
