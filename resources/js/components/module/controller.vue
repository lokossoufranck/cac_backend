          ...
          <div class="col-12">
            ...
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">Liste des controllers </h3>
              ...
              <div class="card-body table-responsive p-0">
                <table class="table table-hover" id="datatable">
                  ...
                  <tbody>
                     <tr v-for="controller in search()" :key="controller.id">
                      <td>{{controller.id}}</td>
                      <td>{{controller.nom}}</td>                    
                      <td>{{controller.mod_nom}}</td>
                      <td v-show="controller.actif"><span   class="badge badge-success"> {{controller.actif}}</span></td>
                      <td v-show="!controller.actif"><span   class="badge badge-danger"> {{controller.actif}}</span></td>
                      <!-- <td><img v-bind:src="'/' + controller.photo" width="100" alt="controller"></td> -->
                      <td>
                        
                        <a href="#" @click="editModal(controller)">
                            <i class="fa fa-edit blue"></i>
                        </a>
                        /
                        <a href="#" @click="deleteController(controller.id)">
              ...
              <div class="card-footer">
                  <pagination :data="controllers" @pagination-change-page="getResults"></pagination>
              </div>
            </div>
            <!-- /.card -->
          </div>
        ...
        <div class="modal fade" id="addNew" tabindex="-1" role="dialog" aria-labelledby="addNew" aria-hidden="true">
            <div class="modal-dialog" role="document">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" v-show="!editmode">Créer un controller</h5>
                    <h5 class="modal-title" v-show="editmode">Modifier controller</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>

                <form @submit.prevent="editmode ? updateController() : createController()">
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label>Code</label>
                            <input v-model="form.code" type="text" name="code" autocomplete="off" maxlength="30"      
...
search() {
      var results = [];
      var searchData = this.controllers.data;
      var sparam = this.query.toLowerCase();
      if(this.query == ""){
       // this.controllers.data = searchData;
       results=searchData;
      }else{
      ...
      return results;
          ...
          getResults(page = 1) {
              ...
              axios.get('/api/controllererp?page=' + page).then(({ data }) => (this.controllers = data.data));

              this.$Progress.finish();
          },
          loadControllers(){
              axios.get("/api/controllererp").then(({ data }) => {
          ...
          createController(){
              ...
              this.form.post('/api/controllererp')
              .then((data)=>{
          ...
          updateController(){
              this.$Progress.start();
              this.form.put('/api/controllererp/'+this.form.id)
              .then((response) => {
                  // success
                  $('#addNew').modal('hide');
                  Toast.fire({
              ...
              Swal.fire({
                  ...
                  confirmButtonText: 'Oui, supprimez-le !'
                      ...
                      // Send request to the server
                        if (result.value) {
                              this.form.delete('/api/controllererp/'+id).then(()=>{
                                      Swal.fire(
                                      'suppression!',
