            ...
            <div class="card">
              ...
              <div class="card-body table-responsive p-0">
                <table class="table table-hover" id="datatable">
                  ...
                  <tbody>
                     <tr v-for="departement in search()" :key="departement.id">
                      <td>{{departement.id}}</td>
                      <td>{{departement.nom}}</td>
                      <td>{{departement.site_nom}}</td>
                      <!-- <td><img v-bind:src="'/' + pays.photo" width="100" alt="pays"></td> -->
                      <td>
                        
                        <a href="#" @click="editModal(departement)">
                            <i class="fa fa-edit blue"></i>
                        </a>
                        /
                        <a href="#" @click="deleteDepartement(departement.id)">
        ...
        <div class="modal fade" id="addNew" tabindex="-1" role="dialog" aria-labelledby="addNew" aria-hidden="true">
            <div class="modal-dialog" role="document">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" v-show="!editmode">Créer un département</h5>
                    <h5 class="modal-title" v-show="editmode">Modifier département</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>

                <form @submit.prevent="editmode ? updateDepartement() : createDepartement()">
...
search() {
      var results = [];
      var searchData = this.departements.data;
      ...
      return results;
          ...
          getResults(page = 1) {
              ...
              axios.get('/api/departement?page=' + page).then(({ data }) => (this.departements = data.data));

              this.$Progress.finish();
          },
          loadDepartements(){
              axios.get("/api/departement").then(({ data }) => {
                this.departements = data.data; 
                }
                );
          },
          editModal(departement){
              this.editmode = true;
              this.form.reset();
              $('#addNew').modal('show');
              this.form.fill(departement);
          },
          newModal(){
              this.editmode = false;
              this.form.reset();
          ...
          createDepartement(){
              ...
              this.form.post('/api/departement')
              .then((data)=>{
          ...
          updateDepartement(){
              this.$Progress.start();
              this.form.put('/api/departement/'+this.form.id)
              .then((response) => {
                  // success
                  $('#addNew').modal('hide');
                  Toast.fire({
              ...
              Swal.fire({
                  ...
                  confirmButtonText: 'Oui, supprimez-le !'
                      ...
                      // Send request to the server
                        if (result.value) {
                              this.form.delete('/api/departement/'+id).then(()=>{
                                      Swal.fire(
                                      'suppression!',
